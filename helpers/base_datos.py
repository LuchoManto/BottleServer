...
def cargar_desde_bd_medicion():
    ...
    db = MySQLdb.connect("localhost", "ignacio", "mantosamba", "SensorCampoElectroEstatico")
    curs = db.cursor()
    curs.execute("SELECT * FROM medicion LIMIT values(%d,999999999999999)", row_numb_medicion)
...
def cargar_desde_bd_comando():
    ...
    db = MySQLdb.connect("localhost", "ignacio", "mantosamba", "SensorCampoElectroEstatico")
    curs = db.cursor()
    curs.execute("SELECT * FROM comandlog LIMIT values(%d ,999999999999999)", row_numb_log)
